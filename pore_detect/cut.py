...
class CutImg:
    ...
    def left_pore_area(self, img):
        keypoint_new = []
        face_center_x = self.keypoint_all[5][0]
        face_center_y = self.keypoint_all[5][1]
        bias = 0.03

        keypoint_right = [423, 355,343, 451, 449]
        keypoint_right = [423, 343, 348, 347]
        for point in keypoint_right:
            if point == 348:
                k_p = [self.keypoint_all[point][0] - 5 * bias * abs(face_center_x - self.keypoint_all[point][0]),
                       self.keypoint_all[point][1] + 5 * bias * abs(face_center_y - self.keypoint_all[point][1])]
            else:
        ...
        keypoint_other = [280, 427, 346]
        keypoint_other = [280, 427,]
        for point in keypoint_other:
            if point == 280:
    ...
    def right_pore_area(self, img):
        ...
        for point in keypoint_left:
            ...
            cv2.putText(img, str(point), tuple(int(i) for i in k_p), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 0, 0), 2)
            cv2.circle(img, tuple(int(i) for i in k_p), 5, (0, 0, 255), -1)
        keypoint_other = [50, 207, 117]
        keypoint_other = [50, 207]
...
if __name__ == "__main__":
    dir_path = r'F:\fh\center_white\0411\selected\input/'
    out_path = r"F:\fh\center_white\0411\selected\cut/"
    ...
    for file in os.listdir(dir_path):
            ...
            for part in parts_list:
                ...
                cv2.imwrite(out_path+name + f"{part_name}_parts.jpg", rect_img)
                ...
                # break
                cv2.imwrite(out_path+name+f"{part_name}_cut.jpg", parts_img)
            # img_cp1 = img.copy()
